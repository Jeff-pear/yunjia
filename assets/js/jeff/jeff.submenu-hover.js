...
(function($ , undefined) {
 ...
 if( jeff.vars['very_old_ie'] ) return;
 //ignore IE7 & below

 var hasTouch = jeff.vars['touch'];
 var nativeScroll = jeff.vars['old_ie'] || hasTouch;
 ...
 $(window).on('resize.sidebar.jeff_hover', function() {
	$('.sidebar[data-sidebar-hover=true]').jeff_sidebar_hover('update_vars').jeff_sidebar_hover('reset');
 })

 $(document).on('settings.jeff.jeff_hover', function(e, event_name, event_val) {
	if(event_name == 'sidebar_collapsed') $('.sidebar[data-sidebar-hover=true]').jeff_sidebar_hover('reset');
	else if(event_name == 'navbar_fixed') $('.sidebar[data-sidebar-hover=true]').jeff_sidebar_hover('update_vars');
 })
 ...
 function Sidebar_Hover(sidebar , settings) {
	...
	var attrib_values = jeff.helper.getAttrSettings(sidebar, $.fn.jeff_sidebar_hover.defaults);
	this.settings = $.extend({}, $.fn.jeff_sidebar_hover.defaults, settings, attrib_values);
	...
	this.reset = function() {
		...
		$sidebar.find('.submenu').each(function() {
			...
			if($sub.hasClass('jeff-scroll')) {
				$sub.jeff_scroll('disable');
			}
			else {
	...
	this.updateStyle = function(newStyle) {
		...
		$sidebar.find('.submenu.jeff-scroll').jeff_scroll('update', {styleClass: newStyle});
	}
	...
	if(!nativeScroll)
	$sidebar.on('hide.jeff.submenu.sidebar_hover', '.submenu', function(e) {
		if(lastScrollHeight < 1) return;

		e.stopPropagation();
		var $sub = $(this).closest('.jeff-scroll.can-scroll');
	...
	//reset scrollbars 
	if(!nativeScroll)
	$sidebar.on('shown.jeff.submenu.sidebar_hover hidden.jeff.submenu.sidebar_hover', '.submenu', function(e) {
		if(lastScrollHeight < 1) return;
		...
		if(lastScrollHeight > 14 && sub_h - lastScrollHeight > 4) {
			$sub.jeff_scroll('enable').jeff_scroll('reset');//don't update track position
		}
		else {
			$sub.jeff_scroll('disable');
		}
	...
	$sidebar.on(event_1, '.nav-list li, .sidebar-shortcuts', function (e) {
		sidebar_vars = $sidebar.jeff_sidebar('vars');
		...
		if(sub) {
		 if(is_abs) {
			...
			var newScroll = jeff.helper.scrollTop();
	...
	function getSubScroll(el, func) {
		var sub_scroll = $(el).data('jeff_scroll');
	...
	function adjust_submenu(sub) {
		...
		//if(  ) {
			...
			var jeff_scroll = false;
			if(!nativeScroll) {
				jeff_scroll = getSubScroll(sub);
				if(jeff_scroll == false) {
					$sub.jeff_scroll({
						//hideOnIdle: true,
						observeContent: true,
						detached: true,
					...
					var track = jeff_scroll.get_track();
					if(track) {
				...
				jeff_scroll.update({size: scroll_height});
			}
			else {
				$sub
				.addClass('sub-scroll')
			...
			if(!nativeScroll && jeff_scroll) {
				...
				if(track) {
					...
					var left = off.left 
					if( !scroll_right ) {
						left += ($sub.outerWidth() - jeff_scroll.track_size());
					}
					else {
						left += 2;
					}
 ...
 /////////////////////////////////////////////
 $.fn.jeff_sidebar_hover = function (option, value) {
	var method_call;

	var $set = this.each(function () {
		var $this = $(this);
		var data = $this.data('jeff_sidebar_hover');
		var options = typeof option === 'object' && option;

		if (!data) $this.data('jeff_sidebar_hover', (data = new Sidebar_Hover(this, options)));
  ...
  $.fn.jeff_sidebar_hover.defaults = {
