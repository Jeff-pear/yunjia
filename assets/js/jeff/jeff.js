...
(function(undefined) {
	if( !('jeff' in window) ) window['jeff'] = {}
	if( !('helper' in window['jeff']) ) window['jeff'].helper = {}
	if( !('vars' in window['jeff']) ) window['jeff'].vars = {}
	window['jeff'].vars['icon'] = ' jeff-icon ';
	window['jeff'].vars['.icon'] = '.jeff-icon';

	jeff.vars['touch']	= ('ontouchstart' in window);//(('ontouchstart' in document.documentElement) || (window.DocumentTouch && document instanceof DocumentTouch));
	
	//sometimes the only good way to work around browser's pecularities is to detect them using user-agents
	//though it's not accurate
	var agent = navigator.userAgent
	jeff.vars['webkit'] = !!agent.match(/AppleWebKit/i)
	jeff.vars['safari'] = !!agent.match(/Safari/i) && !agent.match(/Chrome/i);
	jeff.vars['android'] = jeff.vars['safari'] && !!agent.match(/Android/i)
	jeff.vars['ios_safari'] = !!agent.match(/OS ([4-9])(_\d)+ like Mac OS X/i) && !agent.match(/CriOS/i)
	
	jeff.vars['ie'] = window.navigator.msPointerEnabled || (document.all && document.querySelector);//8-11
	jeff.vars['old_ie'] = document.all && !document.addEventListener;//8 and below
	jeff.vars['very_old_ie']	= document.all && !document.querySelector;//7 and below
	jeff.vars['firefox'] = 'MozAppearance' in document.documentElement.style;
	
	jeff.vars['non_auto_fixed'] = jeff.vars['android'] || jeff.vars['ios_safari'];
	
	
	//sometimes we try to use 'tap' event instead of 'click' if jquery mobile plugin is available
	jeff['click_event'] = jeff.vars['touch'] && jQuery.fn.tap ? 'tap' : 'click';
})();
...
(function($ , undefined) {
	...
	jeff.demo = {
		...
		init: function(initAnyway) {
			...
			for(var func in jeff.demo.functions) if(jeff.demo.functions.hasOwnProperty(func)) {
				jeff.demo.functions[func]();
			}
		}
	}


	jeff.demo.functions.basics = function() {
		// for android and ios we don't use "top:auto" when breadcrumbs is fixed
		if(jeff.vars['non_auto_fixed']) {
			$('body').addClass('mob-safari');
		}

		jeff.vars['transition'] = jeff.vars['animation'] || !!$.support.transition;
	}
	
	jeff.demo.functions.enableSidebar = function() {
		//initiate sidebar function
		var $sidebar = $('.sidebar');
		if($.fn.jeff_sidebar) $sidebar.jeff_sidebar();
		if($.fn.jeff_sidebar_scroll) $sidebar.jeff_sidebar_scroll({
			//for other options please see documentation
			'include_toggle': false || jeff.vars['safari'] || jeff.vars['ios_safari'] //true = include toggle button in the scrollbars
		});
		if($.fn.jeff_sidebar_hover)	$sidebar.jeff_sidebar_hover({
			'sub_hover_delay': 750,
	...
	jeff.demo.functions.enableDemoAjax = function() {
		if(!$.fn.jeff_ajax) return;
		...
		var demo_ajax_options = {
			 ...
			 'content_url': function(hash) {
				//***NOTE***
				//this is for jeff demo only, you should change it to return a valid URL
				//please refer to documentation for more info

				if( !hash.match(/^page\//) ) return false;
				var path = document.location.pathname;

				//for example in jeff HTML demo version we convert /ajax/index.html#page/gallery to > /ajax/content/gallery.html and load it
				if(path.match(/(\/ajax\/)(index\.html)?/))
					return path.replace(/(\/ajax\/)(index\.html)?/, '/ajax/content/'+hash.replace(/^page\//, '')+'.html') ;

				//for example in jeff PHP demo version we convert "ajax.php#page/dashboard" to "ajax.php?page=dashboard" and load it
		...
		//initiate ajax loading on this element( which is .page-content-area[data-ajax-content=true] in jeff's demo)
		$('[data-ajax-content=true]').jeff_ajax(demo_ajax_options)

		//if general error happens and ajax is working, let's stop loading icon & PACE
		$(window).on('error.jeff_ajax', function() {
			$('[data-ajax-content=true]').each(function() {
				var $this = $(this);
				if( $this.jeff_ajax('working') ) {
					if(window.Pace && Pace.running) Pace.stop();
					$this.jeff_ajax('stopLoading', true);
				}
			})
		})
	}
	...
	jeff.demo.functions.handleScrollbars = function() {
		//add scrollbars for navbar dropdowns
		var has_scroll = !!$.fn.jeff_scroll;
		if(has_scroll) $('.dropdown-content').jeff_scroll({reset: false, mouseWheelLock: true})

		//reset scrolls bars on window resize
		if(has_scroll && !jeff.vars['old_ie']) {//IE has an issue with widget fullscreen on ajax?!!!
			$(window).on('resize.reset_scroll', function() {
				$('.jeff-scroll:not(.scroll-disabled)').not(':hidden').jeff_scroll('reset');
			});
			if(has_scroll) $(document).on('settings.jeff.reset_scroll', function(e, name) {
				if(name == 'sidebar_collapsed') $('.jeff-scroll:not(.scroll-disabled)').not(':hidden').jeff_scroll('reset');
			});
		}
	}


	jeff.demo.functions.dropdownAutoPos = function() {
		...
		$(document).on('click.dropdown.pos', '.dropdown-toggle[data-position="auto"]', function() {
			...
			if ( parseInt(offset.top + $(this).height()) + 50 
					>
				(jeff.helper.scrollTop() + jeff.helper.winHeight() - parent.find('.dropdown-menu').eq(0).height()) 
	...
	jeff.demo.functions.navbarHelpers = function() {
		...
		$('.jeff-nav [class*="icon-animated-"]').closest('a').one('click', function(){
	...
	jeff.demo.functions.sidebarTooltips = function() {
	...
	jeff.demo.functions.scrollTopBtn = function() {
		//the scroll to top button
		var scroll_btn = $('.btn-scroll-up');
		if(scroll_btn.length > 0) {
			var is_visible = false;
			$(window).on('scroll.scroll_btn', function() {
				var scroll = jeff.helper.scrollTop();
				var h = jeff.helper.winHeight();
			...
			scroll_btn.on(jeff.click_event, function(){
				var duration = Math.min(500, Math.max(100, parseInt(jeff.helper.scrollTop() / 3)));
	...
	jeff.demo.functions.someBrowserFix = function() {
		//chrome and webkit have a problem here when resizing from 479px to more
		//we should force them redraw the navbar!
		if( jeff.vars['webkit'] ) {
			var jeff_nav = $('.jeff-nav').get(0);
			if( jeff_nav ) $(window).on('resize.webkit_fix' , function(){
				jeff.helper.redraw(jeff_nav);
			});
		}
		
		
		//fix an issue with ios safari, when an element is fixed and an input receives focus
		if(jeff.vars['ios_safari']) {
		  $(document).on('jeff.settings.ios_fix', function(e, event_name, event_val) {
			...
			if(event_val == true) {
			  $(document).on('focus.ios_fix', 'input,textarea,.wysiwyg-editor', function() {
				$(window).on('scroll.ios_fix', function() {
					...
					if(navbar) jeff.helper.redraw(navbar);
				});
			  }).on('blur.ios_fix', 'input,textarea,.wysiwyg-editor', function() {
				$(window).off('scroll.ios_fix');
			  })
			}
		  }).triggerHandler('jeff.settings.ios_fix', ['navbar_fixed', $('#navbar').css('position') == 'fixed']);
		}
	}

	
	
	jeff.demo.functions.bsCollapseToggle = function() {
		...
		$(document).on('hide.bs.collapse show.bs.collapse', function (ev) {
			...
			panel.find(jeff.vars['.icon']).each(function(){
				var $icon = $(this)
	...
	jeff.demo.functions.smallDeviceDropdowns = function() {
	  if(jeff.vars['old_ie']) return;
	  
	  $(document)
	  .on('shown.bs.dropdown.navbar', '.jeff-nav > li.dropdown-modal', function(e) {
	  ...
	  .on('hidden.bs.dropdown.navbar', '.jeff-nav > li.dropdown-modal', function(e) {
	  ...
	  function adjustNavbarDropdown() {
		...
		if( $sub.css('position') == 'fixed' ) {
			...
			var tabbed = false;
			var extra_parts = 0;
			var dropdown_content = $sub.find('.tab-pane.active .dropdown-content.jeff-scroll');
			if(dropdown_content.length == 0) dropdown_content = $sub.find('.dropdown-content.jeff-scroll');
			else tabbed = true;
			...
			if(dropdown_content.length == 1) {
				if(!jeff.vars['touch']) {
					dropdown_content.jeff_scroll('update', {size: height - extra_parts}).jeff_scroll('enable').jeff_scroll('reset');
				}
				else {
					dropdown_content
					.jeff_scroll('disable').css('max-height', height - extra_parts).addClass('overflow-scroll');
				}
			}
	  ...
	  function resetNavbarDropdown($sub) {
	    ...
	    if($sub.length > 0) {
			...
			.find('.dropdown-content').each(function() {
				...
				var size = parseInt($(this).attr('data-size') || 0) || $.fn.jeff_scroll.defaults.size;
				$(this).jeff_scroll('update', {size: size}).jeff_scroll('enable').jeff_scroll('reset');
			})
...
//some jeff helper functions
(function($$ , undefined) {//$$ is jeff.helper
