/**
 <b>jeff custom scroller</b>. It is not as feature-rich as plugins such as NiceScroll but it's good enough for most cases.
*/
(function($ , undefined) {
	var jeff_Scroll = function(element , _settings) {
		var self = this;
		
		var attrib_values = jeff.helper.getAttrSettings(element, $.fn.jeff_scroll.defaults);
		var settings = $.extend({}, $.fn.jeff_scroll.defaults, _settings, attrib_values);
		...
		this.create = function(_settings) {
			...
			if(_settings) settings = $.extend({}, $.fn.jeff_scroll.defaults, _settings);
			...
			this.$element.addClass('jeff-scroll');
			...
			if(settings.mouseWheel) {
				...
				this.$element.on(!!$.event.special.mousewheel ? 'mousewheel.jeff_scroll' : 'mousewheel.jeff_scroll DOMMouseScroll.jeff_scroll', function(event) {
					if(disabled) return;
					...
					if(mouse_track) {
						mouse_track = false;
						$('html').off('.jeff_scroll')
						$(mouse_release_target).off('.jeff_scroll');
			...
			var touchDrag = jeff.vars['touch'] && 'jeff_drag' in $.event.special && settings.touchDrag //&& !settings.touchSwipe;
			//add drag event for touch devices to scroll
			if(touchDrag/** || ($.fn.swipe && settings.touchSwipe)*/) {
				var dir = '', event_name = touchDrag ? 'jeff_drag' : 'swipe';
				this.$element.on(event_name + '.jeff_scroll', function(event) {
					if(disabled) {
			...
			if(observeContent) {
				$track.on('mouseenter.jeff_scroll', function() {
					insideTrack = true;
					checkContentChanges(false);
				}).on('mouseleave.jeff_scroll', function() {
					insideTrack = false;
			...
			this.$element.on('mouseenter.jeff_scroll touchstart.jeff_scroll', function(e) {
				is_dirty = true;
				if(observeContent) checkContentChanges(true);
				else if(settings.hoverReset) self.reset(true);
				
				$track.addClass('scroll-hover');
			}).on('mouseleave.jeff_scroll touchend.jeff_scroll', function() {
		...
		this.destroy = function() {
			...
			this.$element.removeClass('jeff-scroll scroll-disabled scroll-active');
			this.$element.off('.jeff_scroll')
		...
		function mouse_down_bar(e) {
			...
			mouse_track = true;
			$('html').off('mousemove.jeff_scroll').on('mousemove.jeff_scroll', mouse_move_bar)
			$(mouse_release_target).off('mouseup.jeff_scroll').on('mouseup.jeff_scroll', mouse_up_bar);
	...
	$.fn.jeff_scroll = function (option,value) {
		var retval;

		var $set = this.each(function () {
			var $this = $(this);
			var data = $this.data('jeff_scroll');
			var options = typeof option === 'object' && option;

			if (!data) $this.data('jeff_scroll', (data = new jeff_Scroll(this, options)));
	...
	/**
	$(document).on('jeff.settings.jeff_scroll', function(e, name) {
		if(name == 'sidebar_collapsed') $('.jeff-scroll').scroller('reset');
	});
	$(window).on('resize.jeff_scroll', function() {
		$('.jeff-scroll').scroller('reset');
	});
	*/
