...
(function($ , undefined) {
	...
	function jeff_Spinner(element , _options) {
		var attrib_values = jeff.helper.getAttrSettings(element, $.fn.jeff_spinner.defaults);
		var options = $.extend({}, $.fn.jeff_spinner.defaults, _options, attrib_values);
		...
		$element.addClass('spinbox-input form-control text-center').wrap('<div class="jeff-spinner middle">')

		var $parent_div = $element.closest('.jeff-spinner').spinbox(options).wrapInner("<div class='input-group'></div>")
		...
		if(options.on_sides)
		{
			...
			if(jeff.vars['touch'] || options.touch_spinner) {
				$parent_div.addClass('touch-spinner')
				$parent_div.css('width' , width+'px')
			}
			else {
	...
	$.fn.jeff_spinner = function(option, value) {
		var retval;

		var $set = this.each(function() {
			var $this = $(this);
			var data = $this.data('jeff_spinner');
			var options = typeof option === 'object' && option;

			if (!data) {
				options = $.extend({}, $.fn.jeff_spinner.defaults, option);
				$this.data('jeff_spinner', (data = new jeff_Spinner(this, options)));
			}
