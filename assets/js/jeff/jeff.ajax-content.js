...
(function($ , undefined) {
	...
	function AceAjax(contentArea, settings) {
		...
		var attrib_values = jeff.helper.getAttrSettings(contentArea, $.fn.jeff_ajax.defaults);
		this.settings = $.extend({}, $.fn.jeff_ajax.defaults, settings, attrib_values);
		...
		this.getUrl = function(url, hash, manual_trigger, method, data, updateView, extraParams) {
			...
			.done(function(result) {
					...
					if(link_element.length > 0) {
						...
						if(nav.length > 0) {
							...
							nav.closest('.sidebar[data-sidebar-scroll=true]').each(function() {
								var $this = $(this);
								$this.jeff_sidebar_scroll('reset');
								if(manual_trigger == true) $this.jeff_sidebar_scroll('scroll_to_active');//first time only
							})
						}
					}
				}
				...
				if(updateView) setTimeout(function() {
					$('head').find('link.jeff-ajax-stylesheet').remove();

					var main_selectors = ['link.jeff-main-stylesheet', 'link#main-jeff-style', 'link[href*="/jeff.min.css"]', 'link[href*="/jeff.css"]']
					var jeff_style = [];
					for(var m = 0; m < main_selectors.length; m++) {
						jeff_style = $('head').find(main_selectors[m]).first();
						if(jeff_style.length > 0) break;
					}
					
					$contentArea.find('.ajax-append-link').each(function(e) {
						var $link = $(this);
						if ( $link.attr('href') ) {
							var new_link = jQuery('<link />', {type : 'text/css', rel: 'stylesheet', 'class': 'jeff-ajax-stylesheet'})
							if( jeff_style.length > 0 ) new_link.insertBefore(jeff_style);
				...
				var re = /\.(?:\s*)jeff(?:_a|A)jax(?:\s*)\((?:\s*)(?:\'|\")loadScripts(?:\'|\")/;
		...
		function updateBreadcrumbs(link_element) {
			...
			if(breadcrumbs.length > 0 && breadcrumbs.is(':visible')) {
				...
				link_element.parents('.nav li').each(function() {
					...
					link_clone.find('i,.fa,.glyphicon,.jeff-icon,.menu-icon,.badge,.label').remove();
		 ...
		 function updateTitle(link_text) {
			...
			if($title.length > 0) {
				...
				var extra = $.trim(String(document.title).replace(/^(.*)[\-]/, ''));//for example like " - jeff Admin"
		...
		/////////////////
		$(window)
		.off('hashchange.jeff_ajax')
		.on('hashchange.jeff_ajax', function(e, manual_trigger) {
			var hash = $.trim(window.location.hash);
			if(!hash || hash.length == 0) return;
			
			if(self.settings.close_mobile_menu) {
				try {$(self.settings.close_mobile_menu).jeff_sidebar('mobileHide')} catch(e){}
			}
		...
		}).trigger('hashchange.jeff_ajax', [true]);
	...
	$.fn.aceAjax = $.fn.jeff_ajax = function (option, value, value2, value3, value4) {
		var method_call;

		var $set = this.each(function () {
			var $this = $(this);
			var data = $this.data('jeff_ajax');
			var options = typeof option === 'object' && option;

			if (!data) $this.data('jeff_ajax', (data = new AceAjax(this, options)));
	...
	$.fn.aceAjax.defaults = $.fn.jeff_ajax.defaults = {
		content_url: false,
		default_url: false,
