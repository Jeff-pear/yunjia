...
*/
(function($ , undefined) {
	$.fn.jeff_wysiwyg = function($options , undefined) {
		var options = $.extend( {
			speech_button:true,
			wysiwyg:{}
        }, $options);
		...
		var button_defaults =
		{
			...
			'insertImage' : {
				...
				button_text : '<i class="'+ jeff.vars['icon'] + 'fa fa-file"></i> Choose Image &hellip;',
		...
		this.each(function() {
			...
			for(var tb in toolbar_buttons) if(toolbar_buttons.hasOwnProperty(tb)) {
				...
				switch(button.name) {
					case 'font':
						toolbar += ' <a class="btn btn-sm '+className+' dropdown-toggle" data-toggle="dropdown" title="'+button.title+'"><i class="'+ jeff.vars['icon'] + button.icon+'"></i><i class="' + jeff.vars['icon'] + 'fa fa-angle-down icon-on-right"></i></a> ';
					...
					case 'createLink':
						toolbar += ' <div class="btn-group"> <a class="btn btn-sm '+className+' dropdown-toggle" data-toggle="dropdown" title="'+button.title+'"><i class="'+ jeff.vars['icon'] + button.icon+'"></i></a> ';
					...
					case 'viewSource':
						toolbar += ' <a class="btn btn-sm '+className+'" data-view="source" title="'+button.title+'"><i class="'+ jeff.vars['icon'] + button.icon+'"></i></a> ';
					break;
					default:
						toolbar += ' <a class="btn btn-sm '+className+'" data-edit="'+button.name+'" title="'+button.title+'"><i class="'+ jeff.vars['icon'] + button.icon+'"></i></a> ';
					break;
				}
			}
			toolbar += ' </div> ';
			...
			toolbar.find('input[type=file]').prev().on(jeff.click_event, function (e) { 
				$(this).next().click();
			});
			toolbar.find('.wysiwyg_colorpicker').each(function() {
				$(this).jeff_colorpicker({pull_right:true}).change(function(){
