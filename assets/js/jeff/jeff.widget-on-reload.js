...
(function($ , undefined) {
	...
	//when data is finished loading or processing is done you can call $box.trigger('reloaded.jeff.widget')
	$(document).on('reload.jeff.widget', '.widget-box', function (ev) {
		var $box = $(this);
		
		//trigger the reloaded event to remove the spinner icon after 1-2 seconds
		setTimeout(function() {
			$box.trigger('reloaded.jeff.widget');
		}, parseInt(Math.random() * 1000 + 1000));
	});

	//you may want to do something like this:
	/**
	$('#my-widget-box').on('reload.jeff.widget', function(){
		//load new data here
		//and when finished trigger "reloaded" event
		$(this).trigger('reloaded.jeff.widget');
	});
	*/
})(window.jQuery);