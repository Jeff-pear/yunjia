...
(function($ , undefined) {
	...
	function Sidebar(sidebar, settings) {
		...
		var attrib_values = jeff.helper.getAttrSettings(sidebar, $.fn.jeff_sidebar.defaults, 'sidebar-');
		this.settings = $.extend({}, $.fn.jeff_sidebar.defaults, settings, attrib_values);
		...
		var toggleIcon = function(minimized, save) {
			var icon = $(this).find(jeff.vars['.icon']), icon1, icon2;
			if(icon.length > 0) {
				...
				try {
					if(save !== false) jeff.settings.saveState(icon.get(0));
				} catch(e) {}
			}
		}
		...
		this.toggleMenu = function(toggle_btn, save) {
			...
			try {
				if(save) jeff.settings.saveState(sidebar, 'class', 'menu-min', this.minimized);
			} catch(e) {}
			...
			if(jeff.vars['old_ie']) jeff.helper.redraw(sidebar);
			
			
			$(document).trigger('settings.jeff', ['sidebar_collapsed' , this.minimized, sidebar, save]);
		}
		...
		this.toggleResponsive = function(toggle_btn, showMenu) {
			...
			if(toggle_btn) {
				var icon = $(toggle_btn).find(jeff.vars['.icon']), icon1, icon2;
				if(icon.length > 0) {
					icon1 = icon.attr('data-icon1');//the icon for expanded state
					icon2 = icon.attr('data-icon2');//the icon for collapsed state
			...
			$(document).triggerHandler('settings.jeff', ['sidebar_collapsed' , this.minimized]);
		}
		...
		this.$sidebar.on(jeff.click_event+'.jeff.submenu', '.nav-list', function (ev) {
			var nav_list = this;
			...
			if( !link_element.hasClass('dropdown-toggle') ) {//it doesn't have a submenu return
				...
				if(jeff.vars['ios_safari'] && link_element.attr('data-link') !== 'false') {
			...
			if (height_change != 0) {
				if(self.$sidebar.attr('data-sidebar-scroll') == 'true' && !self.minimized) 
					self.$sidebar.jeff_sidebar_scroll('prehide', height_change)
			}
		...
		this.show = function(sub, $duration, shouldWait) {
			...
			var $sub = $(sub);
			var event;
			$sub.trigger(event = $.Event('show.jeff.submenu'))
			...
			var complete = function(ev, trigger) {
				...
				//if(jeff.vars['webkit']) jeff.helper.redraw(sub);//little Chrome issue, force redraw ;)

				if(trigger !== false) $sub.trigger($.Event('shown.jeff.submenu'))
			...
			if($duration == 0 || finalHeight == 0 || !$.support.transition.end) {
				...
				if(jeff.vars['android'] ) {
					setTimeout(function() {
						complete(null, false);
						jeff.helper.redraw(sub);
					}, $duration + 20);
				}
			}
		 ...
		 this.hide = function(sub, $duration, shouldWait) {
			...
			var $sub = $(sub);
			var event;
			$sub.trigger(event = $.Event('hide.jeff.submenu'))
			if (event.isDefaultPrevented()) {
				return false;
			}
			...
			var complete = function(ev, trigger) {
				...
				if(trigger !== false) $sub.trigger($.Event('hidden.jeff.submenu'))
	...
	//menu-toggler
	$(document)
	.on(jeff.click_event+'.jeff.menu', '.menu-toggler', function(e){
		var btn = $(this);
		...
		sidebar.jeff_sidebar('toggleMobile', this);
		
		var click_event = jeff.click_event+'.jeff.autohide';
		...
		if( btn.hasClass('display') ) {
			...
			if(auto_hide) {
				$(document).on(click_event, function(ev) {
					...
					sidebar.jeff_sidebar('toggleMobile', this, false);
					$(document).off(click_event);
				})
			}

			if(sidebar.attr('data-sidebar-scroll') == 'true') sidebar.jeff_sidebar_scroll('reset');
		}
		else {
	...
	.on(jeff.click_event+'.jeff.menu', '.sidebar-collapse', function(e){
		...
		e.preventDefault();
		$sidebar.jeff_sidebar('toggleMenu', this);
	})
	//this button is used in `mobile_style = 3` responsive menu style to expand minimized sidebar
	.on(jeff.click_event+'.jeff.menu', '.sidebar-expand', function(e){
		var target = $(this).attr('data-target'), $sidebar = null;
		if(target) $sidebar = $(target);
		if($sidebar == null || $sidebar.length == 0) $sidebar = $(this).closest('.sidebar');
		if($sidebar.length == 0) return;	
		...
		var btn = this;
		e.preventDefault();
		$sidebar.jeff_sidebar('toggleResponsive', this);
		...
		if($sidebar.attr('data-auto-hide') === 'true') {
			if( $sidebar.hasClass(responsive_max_class) ) {
				$(document).on(click_event, function(ev) {
					...
					$sidebar.jeff_sidebar('toggleResponsive', btn);
					$(document).off(click_event);
				})
			}
			else {
	...
	$.fn.jeff_sidebar = function (option, value, value2) {
		var method_call;

		var $set = this.each(function () {
			var $this = $(this);
			var data = $this.data('jeff_sidebar');
			var options = typeof option === 'object' && option;

			if (!data) $this.data('jeff_sidebar', (data = new Sidebar(this, options)));
