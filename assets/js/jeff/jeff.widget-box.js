...
(function($ , undefined) {
	...
	var Widget_Box = function(box, options) {
		...
		this.reload = function() {
			...
			$box.append('<div class="widget-box-overlay"><i class="'+ jeff.vars['icon'] + 'loading-icon fa fa-spinner fa-spin fa-2x white"></i></div>');

			$box.one('reloaded.jeff.widget', function() {
		...
		this.close = function(closeSpeed) {
			...
			$box.fadeOut(closeSpeed , function(){
					$box.trigger('closed.jeff.widget');
					$box.remove();
				}
			)
		}
		...
		this.toggle = function(type, button, expandSpeed, collapseSpeed) {
			...
			if(button) {
				...
				$icon = button.find(jeff.vars['.icon']).eq(0);
			...
			if( event_name == 'show' ) {
				...
				$body.slideDown(expandSpeed, function(){
					$box.trigger(event_complete_name+'.jeff.widget')
				})
			}
			else {
		...
		this.fullscreen = function() {
			var $icon = this.$box.find('> .widget-header a[data-action=fullscreen]').find(jeff.vars['.icon']).eq(0);
			...
			this.$box.trigger('fullscreened.jeff.widget')
		}
	...
	$(document).on('click.jeff.widget', '.widget-header a[data-action]', function (ev) {
		ev.preventDefault();
		...
		if($action == 'collapse') {
			...
			var event
			$box.trigger(event = $.Event(event_name+'.jeff.widget'))
			...
			var event
			$box.trigger(event = $.Event('close.jeff.widget'))
			if (event.isDefaultPrevented()) return
			...
			$this.blur();
			var event
			$box.trigger(event = $.Event('reload.jeff.widget'))
			if (event.isDefaultPrevented()) return
			...
			var event
			$box.trigger(event = $.Event('fullscreen.jeff.widget'))
			if (event.isDefaultPrevented()) return
		
			$widget_box.fullscreen();
		}
		else if($action == 'settings') {
			$box.trigger('setting.jeff.widget')
		}
	...
	function applyScrollbars($widget, enable) {
		...
		var nativeScrollbars = jeff.vars['old_ie'] || jeff.vars['touch'];
		
		if(enable) {
			var jeff_scroll = $main.data('jeff_scroll');
			if( jeff_scroll ) {
				$main.data('save_scroll', {size: jeff_scroll['size'], lock: jeff_scroll['lock'], lock_anyway: jeff_scroll['lock_anyway']});
			}
			...
			if( !nativeScrollbars ) {
				if( jeff_scroll ) {
					$main.jeff_scroll('update', {'size': size, 'mouseWheelLock': true, 'lockAnyway': true});
				}
				else {
					$main.jeff_scroll({'size': size, 'mouseWheelLock': true, 'lockAnyway': true});
				}
				$main.jeff_scroll('enable').jeff_scroll('reset');
			}
			else {
				if( jeff_scroll ) $main.jeff_scroll('disable');
			...
			.on('resize.widget.scroll', function() {
				...
				$main.css('min-height', size);
				if( !nativeScrollbars ) {
					$main.jeff_scroll('update', {'size': size}).jeff_scroll('reset');
				}
				else {
					$main.css('max-height', size).addClass('overflow-scroll');
				}
			...
			if(saved_scroll) {
				$main
				.jeff_scroll('update', {'size': saved_scroll['size'], 'mouseWheelLock': saved_scroll['lock'], 'lockAnyway': saved_scroll['lock_anyway']})
				.jeff_scroll('enable')
				.jeff_scroll('reset');
			}
			
			if( !nativeScrollbars ) {				
				if(!saved_scroll) $main.jeff_scroll('disable');				
			}
			else {
