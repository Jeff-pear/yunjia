...
(function($ , undefined) {
	...
	var jeff_File_Input = function(element , settings) {
		var self = this;
		
		var attrib_values = jeff.helper.getAttrSettings(element, $.fn.jeff_file_input.defaults);
		this.settings = $.extend({}, $.fn.jeff_file_input.defaults, settings, attrib_values);
		...
		this.$element
		.off('change.jeff_inner_call')
		.on('change.jeff_inner_call', function(e , jeff_inner_call){
			if(self.disabled) return;
		
			if(jeff_inner_call === true) return;//this change event is called from above drop event and extra checkings are taken care of there
	...
	jeff_File_Input.prototype.apply_settings = function() {
		var self = this;
		...
		if(this.well_style) {
			...
			this.$element.parent().addClass('jeff-file-multiple');
		}
		else {
			this.$element.parent().removeClass('jeff-file-multiple');
		}


		this.$element.parent().find(':not(input[type=file])').remove();//remove all except our input, good for when changing settings
		this.$element.after('<span class="jeff-file-container" data-title="'+this.settings.btn_choose+'"><span class="jeff-file-name" data-title="'+this.settings.no_file+'">'+(this.settings.no_icon ? '<i class="'+ jeff.vars['icon'] + this.settings.no_icon+'"></i>' : '')+'</span></span>');
		this.$label = this.$element.next();
		this.$container = this.$element.closest('.jeff-file-input');

		var remove_btn = !!this.settings.icon_remove;
		if(remove_btn) {
			var btn = 
			$('<a class="remove" href="#"><i class="'+ jeff.vars['icon'] + this.settings.icon_remove+'"></i></a>')
			.appendTo(this.$element.parent());

			btn.on(jeff.click_event, function(e){
				e.preventDefault();
	...
	jeff_File_Input.prototype.show_file_list = function($files , inner_call) {
		var files = typeof $files === "undefined" ? this.$element.data('jeff_input_files') : $files;
		if(!files || files.length == 0) return;
		
		//////////////////////////////////////////////////////////////////
		
		if(this.well_style) {
			this.$label.find('.jeff-file-name').remove();
		...
		for (var i = 0; i < files.length; i++) {
			...
			if(!this.well_style) this.$label.find('.jeff-file-name').attr({'data-title':filename}).find(jeff.vars['.icon']).attr('class', jeff.vars['icon'] + fileIcon);
			else {
				this.$label.append('<span class="jeff-file-name" data-title="'+filename+'"><i class="'+ jeff.vars['icon'] + fileIcon+'"></i></span>');
	...
	jeff_File_Input.prototype.reset_input = function() {
	    this.reset_input_ui();
		this.reset_input_field();
	}
	
	jeff_File_Input.prototype.reset_input_ui = function() {
		 this.$label.attr({'data-title':this.settings.btn_choose, 'class':'jeff-file-container'})
			.find('.jeff-file-name:first').attr({'data-title':this.settings.no_file , 'class':'jeff-file-name'})
			.find(jeff.vars['.icon']).attr('class', jeff.vars['icon'] + this.settings.no_icon)
			.prev('img').remove();
			if(!this.settings.no_icon) this.$label.find(jeff.vars['.icon']).remove();
		
		this.$label.find('.jeff-file-name').not(':first').remove();
		
		this.reset_input_data();
		
		//if(jeff.vars['old_ie']) jeff.helper.redraw(this.$container[0]);
	}
	jeff_File_Input.prototype.reset_input_field = function() {
	...
	jeff_File_Input.prototype.reset_input_data = function() {
		if(this.$element.data('jeff_input_files')) {
			this.$element.removeData('jeff_input_files');
			this.$element.removeData('jeff_input_method');
		}
	}

	jeff_File_Input.prototype.enable_reset = function(can_reset) {
		this.can_reset = can_reset;
	}

	jeff_File_Input.prototype.disable = function() {
		this.disabled = true;
		this.$element.attr('disabled', 'disabled').addClass('disabled');
	}
	jeff_File_Input.prototype.enable = function() {
		this.disabled = false;
		this.$element.removeAttr('disabled').removeClass('disabled');
	}

	jeff_File_Input.prototype.files = function() {
		return $(this).data('jeff_input_files') || null;
	}
	jeff_File_Input.prototype.method = function() {
		return $(this).data('jeff_input_method') || '';
	}
	
	jeff_File_Input.prototype.update_settings = function(new_settings) {
		this.settings = $.extend({}, this.settings, new_settings);
		this.apply_settings();
	}
	
	jeff_File_Input.prototype.loading = function(is_loading) {
		if(is_loading === false) {
			this.$container.find('.jeff-file-overlay').remove();
			this.element.removeAttribute('readonly');
		}
		else {
			var inside = typeof is_loading === 'string' ? is_loading : '<i class="overlay-content fa fa-spin fa-spinner orange2 fa-2x"></i>';
			var loader = this.$container.find('.jeff-file-overlay');
			if(loader.length == 0) {
				loader = $('<div class="jeff-file-overlay"></div>').appendTo(this.$container);
	...
	var enable_drop_functionality = function() {
		...
		.on('drop', function(e){
			...
			self.$element.data('jeff_input_method', 'drop');
			self.$element.data('jeff_input_files', file_list);//save files data to be used later by user

			self.show_file_list(file_list , true);
			
			self.$element.triggerHandler('change' , [true]);//true means jeff_inner_call
			return true;
		});
	}
	...
	var handle_on_change = function() {
		...
		this.$element.data('jeff_input_method', 'select');
		this.$element.data('jeff_input_files', file_list);
	...
	var preview_image = function(file) {
		var self = this;
		var $span = self.$label.find('.jeff-file-name:last');//it should be out of onload, otherwise all onloads may target the same span because of delays
		...
		var imgLoaded = function(img, $file) {
			...
			if(thumb == null) {
				...
				$(this).remove();
				deferred.reject({code:jeff_File_Input.error['THUMBNAIL_FAILED']});
				return;
			}
		...
		var imgFailed = function(img) {
			...
			deferred.reject({code:jeff_File_Input.error['IMAGE_LOAD_FAILED']});
		}
		...
		if(hasFile && file instanceof File) {
			...
			reader.onerror = function (e) {
				deferred.reject({code:jeff_File_Input.error['FILE_LOAD_FAILED']});
			}
	...
	var processFiles = function(file_list, dropped) {
		...
		if( !ret || ret.length == 0 ) {
			if( !this.$element.data('jeff_input_files') ) this.reset_input();
	...
	///////////////////////////////////////////
	$.fn.jeffFileInput = $.fn.jeff_file_input = function (option,value) {
		var retval;

		var $set = this.each(function () {
			var $this = $(this);
			var data = $this.data('jeff_file_input');
			var options = typeof option === 'object' && option;

			if (!data) $this.data('jeff_file_input', (data = new jeff_File_Input(this, options)));
