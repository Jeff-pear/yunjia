...
(function($ , undefined) {
	...
	var old_safari = jeff.vars['safari'] && navigator.userAgent.match(/version\/[1-5]/i)
	//NOTE
	...
	function Sidebar_Scroll(sidebar , settings) {
		...
		var attrib_values = jeff.helper.getAttrSettings(sidebar, $.fn.ace_sidebar_scroll.defaults);
		...
		var available_height = function() {
			...
			if(self.sidebar_fixed) offset.top -= jeff.helper.scrollTop();
		...
		var initiate = function(on_page_load) {
			...
			.closest('.jeff-scroll').addClass('nav-scroll');
			...
			*/
			...
			if(jeff.vars['android']) {
				...
				var last_height = jeff.helper.winHeight() , new_height;
				$(window).on('scroll.ace_scroll', function() {
					if(self.is_scrolling && ace_scroll.is_active()) {
						new_height = jeff.helper.winHeight();
		...
		$sidebar.on('hidden.jeff.submenu.sidebar_scroll shown.jeff.submenu.sidebar_scroll', '.submenu', function(e) {
			e.stopPropagation();
	...
	$(document).on('settings.jeff.sidebar_scroll', function(ev, event_name, event_val){
	...
	$(window).on('resize.jeff.sidebar_scroll', function(){
		$('.sidebar[data-sidebar-scroll=true]').each(function() {
			var $this = $(this);
			if( $this.attr('data-sidebar-hover') == 'true' ) $this.ace_sidebar_hover('reset');
			/////////////
