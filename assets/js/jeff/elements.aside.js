...
(function($ , undefined) {
	...
	function Aside(modal, settings) {
		...
		var attrib_values = jeff.helper.getAttrSettings(modal, $.fn.jeff_aside.defaults);
		this.settings = $.extend({}, $.fn.jeff_aside.defaults, settings, attrib_values);
		...
		this.hide = function() {
			...
			if(jeff.vars['transition'] && !hasFade) {
		...
		this.shown = function() {
			...
			if(!jeff.vars['touch']) {
				if(!content.hasClass('jeff-scroll')) {
					content.jeff_scroll({
							size: size,
							reset: true,
							mouseWheelLock: true,
			...
			.on('resize.modal.aside', function() {
				if(!jeff.vars['touch']) {
				  content.jeff_scroll('disable');//to get correct size when going from small window size to large size
					var size = !vertical ? dialog.height() : content.height();
					content
					.jeff_scroll('update', {'size': size})
					.jeff_scroll('enable')
					.jeff_scroll('reset');
				}
			...
			///////////////////////////////////////////////////////////////////////////
			if(self.container && jeff.vars['transition'] && !hasFade) {
				$modal.one('bsTransitionEnd', function() {
		...
		this.insideContainer = function() {
			...
			if( container.hasClass('container') ) {
				...
				if(flag && jeff.vars['firefox']) {
					jeff.helper.redraw(container[0]);
				}
			}
		}
		...
		var toggleButton = function() {
			...
			var icon = btn.find(jeff.vars['.icon']);
	...
	$(window).on('resize.aside_container', function() {
		$('.modal.aside').jeff_aside('insideContainer');
	});
	$(document).on('settings.jeff.aside', function(e, event_name) {
		if(event_name == 'main_container_fixed') $('.modal.aside').jeff_aside('insideContainer');
	});

	$.fn.aceAside = $.fn.jeff_aside = function (option, value) {
		var method_call;

		var $set = this.each(function () {
			var $this = $(this);
			var data = $this.data('jeff_aside');
			var options = typeof option === 'object' && option;

			if (!data) $this.data('jeff_aside', (data = new Aside(this, options)));
