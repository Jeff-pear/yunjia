    ...
    $.extend(Image.prototype, {
        ...
        **/
        render: function() {
			...
			this.$file.jeff_file_input(this.options.image).on('change', function(){
				var $rand = (self.$file.val() || self.$file.data('jeff_input_files')) ? Math.random() + "" + (new Date()).getTime() : null;
...
(function ($) {
    ...
    $.extend(Wysiwyg.prototype, {
        render: function () {
			...
			this.$tpl.parent().find('.wysiwyg-editor').show().jeff_wysiwyg(this.options.wysiwyg)
       ...
       activate: function() {
            if(this.$input.is(':visible')) {
				...
				if(!this.nativeUI) {
					...
					this.$input.jeff_spinner(options);
				}
				else {
